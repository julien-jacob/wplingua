...
function wplng_ajax_heartbeat() {
	...
	$slugs                          = wplng_get_slugs();
	$to_translate_count_by_language = array();
	$target_language_ids            = wplng_get_languages_target_ids();
	$to_translate_by_language       = array();


	foreach ( $slugs as $key => $slug ) {

		foreach ( $target_language_ids as $key => $language_id ) {
			if ( ! isset( $slug['translations'][ $language_id ] ) ) {
				$to_translate_by_language[ $language_id ][] = $slug['source'];
			}
		}
	}

	// Variable pour stocker le langage avec le plus d'éléments
	$max_language_id = null;
	$max_count       = 0;

	// Parcourir les sous-tableaux pour trouver celui avec le plus d'éléments
	foreach ( $to_translate_by_language as $language_id => $translations ) {
		$count = count( $translations );
		if ( $count > $max_count ) {
			$max_count       = $count;
			$max_language_id = $language_id;
		}
	}

	// Afficher le résultat
	echo '<pre>';
	echo 'Language ID with the most translations: ' . $max_language_id . "\n";
	echo 'Number of translations: ' . $max_count . "\n";
	echo '</pre>';
	...
	foreach ( $to_translate_by_language[ $max_language_id ] as $key => $slug ) {

		++$curent_count;

		if ( $curent_count >= WPLNG_MAX_TRANSLATIONS_STR ) {
			break;
		}

		$current_length += strlen( $slug ) + 8;

		if ( $current_length > WPLNG_MAX_TRANSLATIONS_CHAR ) {
			break;
		}

		$text_to_translate[] = str_replace(
			array( '-', '_' ),
			' ',
			$slug
		);
	...
	echo '<pre>';
	var_dump( $texts_translated );
	echo '</pre>';

	// echo '<pre>';
	// var_dump( wplng_get_language_website_id() );
	// echo '</pre>';

	// echo '<pre>';
	// var_dump( $max_language_id );
	// echo '</pre>';

	// echo '<pre>';
	// var_dump( $text_to_translate );
	// echo '</pre>';
